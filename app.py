...
を目標として欲しいです!!!←⚠️下の選択で"はい"を押したのに関わらずこれできてない人は、シフト全部いけるとみなしますよほんま
⚠️第二希望も予定ない限り入れそうな所全部提出してください。（選手権など都合がある場合は考慮します）
</li>
...
<small>
※1日のシフトが
<strong>6時間を超える場合は1時間以上の休憩</strong>
を必ず挟むように自動で調整されます。
</small>
</li>
...
with st.expander("📖 初めての方はこちら（シフト提出のやり方）", expanded=True):
    st.markdown("""
### 📝 シフト提出は「2段階」です
...
希望するシフトをすべて追加したら、画面下の
**「以上の内容で提出する」** を必ず押してください。

⚠️ **このボタンを押すまで、シフト希望に反映されないです。**

### 提出完了までの流れ

`名前を選ぶ` → `希望シフトを選ぶ` → `➕ リストに追加` → `内容を確認` → **`以上の内容で提出する`**

💡 **「リストに追加」だけで終わらず、最後の「以上の内容で提出する」まで押してください。**
    """)
...
with st.container(border=True):
    ...
    st.info(
        "⚠️ このボタンは『提出』ではありません。希望をリストに追加したあと、最後に『以上の内容で提出する』を押してください。"
    )
...
if st.session_state.shift_list:
    st.warning(
        "⚠️ まだ正式提出していません！\n\n"
        "リストに追加したシフトを確認して、"
        "下の『以上の内容で提出する』を押してください。"
    )
...
with refresh_col1:
    if st.button(
        "🔄 最新の提出状況に更新",
        use_container_width=True,
        key="refresh_submission_status"
    ):
        # Streamlitを再実行すると、冒頭でGoogle Sheetsから
        # 最新データを読み直すため、他の人の提出内容も反映される。
        st.rerun()

with refresh_col2:
    st.caption("他人が提出した内容を確認したいときに押してください(自身が提出ボタンを押しても反映されてない時も押してみてください)")
