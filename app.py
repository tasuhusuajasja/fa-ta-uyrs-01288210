...
st.markdown("希望する日時を追加して、最後に提出してください。（完全同期・PEM強力修復版）")
    ...
    for idx, tab in enumerate(tabs):
        with tab:
            ...
            if len(day_shifts) == 0:
                ...
                col_name = "开始" if "開始" not in df_timeline.columns and "开始" in df_timeline.columns else "開始"
                df_timeline["開始_dt"] = pd.to_datetime(df_timeline[col_name])
                df_timeline["終了_dt"] = pd.to_datetime(df_timeline["終了"])
        ...
        with st.spinner("計算中... ルールと希望を調整しています..."):
            ...
            if not df_final.empty:
                ...
                for _, row in df_final.iterrows():
                    col_start = "开始" if "開始" not in row and "开始" in row else "開始"
